...
export default function ConnectPrivatePage() {
  ...
  async function handleSubmit() {
    ...
    const res = await fetch("/api/connect/private", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ locationId: locationId.trim(), token: token.trim(), label: label.trim() }),
    });
    ...
    <main className="flex-1 flex items-center justify-center px-6 py-16">
      <div className="w-full max-w-md">
        ...
        <h1 className="mt-4 text-xl font-semibold text-foreground">
          Connect via Private Integration
        </h1>
        <p className="mt-2 text-sm leading-6 text-muted">
          Create a Private Integration token inside the sub-account:{" "}
          <span className="text-foreground">Settings → Integrations → Private Integrations → Add</span>.
          Scope it to at least <code className="rounded bg-background px-1 font-mono text-xs">calendars</code>.
        </p>
